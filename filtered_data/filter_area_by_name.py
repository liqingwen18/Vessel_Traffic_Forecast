...
def extract_records_by_keywords(file_path, keywords, output_file_path, start_line=0):
    """基于关键词从AIS数据中提取记录"""
    columns = [
            ...
            with open(output_file_path, mode, encoding='utf-8', newline='') as out_f:
                ...
                for row in tqdm(csv_reader, total=line_count-start_line, 
                    ...
                    if len(row) >= 26:
                        ...
                        try:
                            # 检查船名
                            shipname = row[5].lower() if row[5] else ""
                            # 检查目的地
                            destination = row[12].lower() if row[12] else ""
                            # 检查港口名称
                            port_name = row[15].lower() if row[15] else ""
                            
                            # 如果任何一个字段包含关键词，则筛选该记录
                            if any(keyword in shipname or keyword in destination or keyword in port_name 
...
def main():
    # 关键词列表，用于筛选可能在浔江流域的记录
    keywords = [
        "浔江", "浔州", "苍梧", "梧州", "藤县", "蒙山", "贺州", "昭平",
        "贵港", "桂平", "平南", "梧州港", "贵港港", "浔江货运", "浔江船运"
    ]
    print(f"将使用以下关键词筛选AIS数据: {', '.join(keywords)}")
    
    # 创建输出目录
    output_dir = 'filtered_keyword_data'
    ...
    for file_path in ais_files:
        ...
        # 处理文件
        filtered_count = extract_records_by_keywords(file_path, keywords, output_file, start_line)
