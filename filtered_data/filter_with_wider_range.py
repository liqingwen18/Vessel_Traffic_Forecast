...
def extract_records_by_coordinates(file_path, min_lon, max_lon, min_lat, max_lat, 
                                  output_file_path, start_line=0, coordinate_divisors=[1000000, 10000000]):
    """基于多种坐标转换方法和扩大的坐标范围从AIS数据中提取记录"""
    columns = [
                ...
                for row in tqdm(csv_reader, total=line_count-start_line, 
                    ...
                    if len(row) >= 20:  # 至少要有经纬度数据
                        try:
                            ...
                            # 尝试多种坐标转换方法
                            is_in_range = False
                            for divisor in coordinate_divisors:
                                real_lon = lon / divisor
                                real_lat = lat / divisor
...
def main():
    ...
    # 坐标转换因子列表，会尝试每一个
    coordinate_divisors = [1000000, 10000000]
    
    print(f"将筛选以下坐标范围内的AIS数据:")
    print(f"经度: {min_lon} ~ {max_lon}")
    print(f"纬度: {min_lat} ~ {max_lat}")
    print(f"坐标转换因子: {coordinate_divisors}")
    
    # 创建输出目录
    output_dir = 'filtered_wide_range'
    ...
    for file_path in ais_files:
        ...
        filtered_count = extract_records_by_coordinates(
            ...
            output_file, start_line, coordinate_divisors
        )
...
def merge_results(output_dir):
    ...
    merged_file = f"广西区域AIS数据_{timestamp}.csv"
